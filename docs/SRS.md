...
* **السيناريو الثاني: فشل الحجز بسبب خرق وقت التعقيم (تدفق خاطئ - Invalid Input)**
  ...
  * *الاستجابة:* محاولة حجز عملية ثانية تبدأ 11:15(الفارق 15 دقيقة وهو اقل من وقت التعقيم الالزامي 45 دقيقة).
...
تصميم المخطط الهندسي التكاملي للموديول على منصة Draw.io :

https://drive.google.com/file/d/1R4uqBBfvsWN8nCmqOo2FKa0HadRnxaCP/view?usp=sharing

لتوضيح كيفية ربط موديول الجراحة مع الأنظمة الأخرى (القبول، الأسرة، والفوترة) وقاعدة البيانات المركزية عبر واجهات برمجية معتمدة
صياغة المتطلبات الوظيفية للموديول (Functional Requirements):
تحديد وهندسة 10 متطلبات وظيفية دقيقة (من FR-1 إلى FR-10) تغطي دورة حياة الحجز الجراحي كاملة، بدءاً من إدخال البيانات وحساب وقت التعقيم آلياً وحتى تأمين الموارد.
إعداد جدول توصيف حالة الاستخدام (Use Case Specification):
بناء وثيقة وجدول التحليل الشامل لحالة الاستخدام الرئيسية للموديول المعرفة بـ (UC-05: Schedule Surgery & Resource Validation)، وتحديد الفاعلين، الشروط المسبقة، والشروط اللاحقة للنظام بدقة.
تحليل وتوثيق سيناريوهات تدفق النظام (Use Case Scenarios):
ابتكار وتوثيق التدفق الأساسي الناجح (Basic Flow) لرحلة البيانات، وصياغة التدفقات البديلة والاستثنائية (Alternative & Exception Flows) لمعالجة مشكلات تضارب الموارد (مثل تضارب الطاقم، أو عدم توفر الأسرة، أو تداخل وقت التعقيم).
كتابة كود التحقق والحوكمة الخلفي (Backend Validation Controller):
قامت الطالبة بترجمة كافة المتطلبات والسيناريوهات التحليلية السابقة إلى كود برمجي حقيقي ونظيف بلغة سي شارب (#C). يمثل هذا الكود وحدة التحكم (Controller) المسؤولة عن فحص وتطبيق الشروط اللوجستية بدقة ومنع حفظ أي بيانات متضاربة في قاعدة البيانات.
ثانياً: التوثيق الفني والتطبيقي الكامل للموديول
1. رابط مخطط المكونات والتكامل المشترك (Component Diagram) على Draw.io
2. المتطلبات الوظيفية لموديول تحسين الجراحة (Functional Requirements)
FR-1: يوفر النظام واجهة تمكن المستخدم المخول من إدخال بيانات طلب الجدولة (رقم المريض، رقم الغرفة، نوع العملية، الطاقم الطبي، والوقت المقترح).
FR-2: يحفظ النظام حجز العملية في قاعدة البيانات المركزية وتغيير حالتها إلى مجدول (Scheduled) فور اجتياز شروط التحقق.
FR-3: يتكامل النظام آلياً مع موديول القبول للتحقق من وجود المريض وصلاحية سجله الطبي عبر الرقم التعريفي الموحد.
FR-4: يمنع النظام جدولة العمليات للمرضى ذوي المخاطر الصحية العالية إلا بعد إدراج موافقة طبيب التخدير المخول.
FR-5: يستعلم النظام تلقائياً من موديول الأسرة للتأكد من توفر سرير شاغر مجهز للمريض في جناح الإقامة لما بعد الجراحة.
FR-6: يحجز النظام السرير للمريض في موديول الأسرة تزامناً مع تأكيد حجز غرفة العمليات.
FR-7: يحسب النظام وقت التعقيم والتطهير الإلزامي تلقائياً بناءً على تصنيف الجراحة (30 دقيقة للعمليات القياسية، و45 دقيقة للعمليات الكبرى).
FR-8: يحظر النظام غرفة العمليات المحددة خلال نافذة التعقيم ويمنع جدولة أي عملية أخرى فيها خلال هذه الفترة.
FR-9: يفحص النظام جداول الطاقم الطبي المحدد (الجراحين وأطباء التخدير) ويمنع الحجز في حال وجود تضارب في مواعيدهم.
FR-10: يتحقق النظام من جاهزية وتوفر الأدوات والأجهزة الطبية المعقمة المطلوبة لنوع الجراحة المحددة قبل تأكيد الحجز.
3. جدول توصيف حالة الاستخدام( Use Case)(Qamar Idres)
الشروط المسبقة
1. تسجيل الدخول بالنظام بصلاحيات مخولة.
2. المريض مسجل مسبقاً في نظام المستشفى وله رقم تعريفي موحد.
الشروط اللاحقة
1. تغيير حالة العملية إلى مجدول وحفظها بقاعدة البيانات.
2. حظر الغرفة لوقت التعقيم الإلزامي الفاصل.
3. حجز وتأمين السرير للمريض بعد الجراحة.
العلاقات المضمنة
تشمل التحقق من المريض (Verify Patient)، التحقق من الموارد والتعقيم (Verify Resources & Sterilization).
4. سيناريوهات تدفق النظام (Use Case Scenarios)
التدفق الأساسي الناجح (Basic Flow):
يقوم المستخدم بإدخال بيانات الجراحة (رقم المريض، رقم الغرفة، الوقت المقترح، والطاقم الطبي).
يستعلم النظام آلياً من نظام القبول ويتأكد من وجود المريض وصلاحية رقمه التعريفي.
يفحص النظام جدول الغرفة وجدول الطاقم الطبي للتأكد من عدم وجود أي تضارب في المواعيد.
يحسب النظام وقت التعقيم الإلزامي (45 دقيقة) ويقوم بحظره في الجدول فوراً بعد وقت العملية لمنع التداخل.
يرسل النظام طلباً لنظام الأسرة ويقوم بتخصيص سرير متاح لإقامة المريض لما بعد الجراحة.
يتم حفظ الموعد في قاعدة البيانات وتتغير حالته إلى مجدول (Scheduled) وتظهر رسالة نجاح للمستخدم.
التدفقات البديلة والاستثنائية (Alternative & Exception Flows):
البديل أ (فشل التحقق من المريض): في الخطوة 2، إذا تبين أن رقم المريض غير مسجل في المستشفى، يتم إيقاف العملية وإظهار رسالة خطأ تفيد بأن رقم المريض غير صالح أو غير مسجل.
البديل ب (تضارب مواعيد الطاقم): في الخطوة 3، إذا تبين أن الجراح أو طبيب التخدير مرتبط بعملية أخرى في نفس الوقت، يرفض النظام الحفظ ويظهر رسالة خطأ تفيد بوجود تضارب في مواعيد الطاقم الطبي المحدد.
البديل ج (تداخل فترة التعقيم): في الخطوة 4، إذا تقاطع الوقت المقترح مع نافذة التعقيم الخاصة بعملية سابقة، يمنع النظام الحجز ويظهر رسالة خطأ تفيد بأن الغرفة قيد التعقيم والتطهير حالياً.
البديل د (عدم توفر أسرة شاغرة)

ثالثاً: الكود البرمجي المعتمد للتحقق والحوكمة (Backend Validation Logic)
كتابة وتطوير الدالة البرمجية التالية بلغة سي شارب (#C) لتطبيق كافة شروط التحقق والسيناريوهات السابقة برمجياً داخل النظام:C#
